   ...
   /**
    ...
    public function index(Request $request)
    {
        ...
        if($request->get('latitude') && $request->get('longitude'))
        {
            ...
            $distanceStores  = DB::select("SELECT id, ( 6371 * acos( cos( radians($lat) ) * cos( radians( `latitude` ) ) * cos( radians( `longitude` ) - radians($long
                    ) ) + sin( radians($lat) ) * sin( radians( `latitude` ) ) ) ) AS distance
                FROM data_enterprise_detail
                ORDER BY distance ASC");

            $distanceStores = collect($distanceStores);

            $stores = [];

            foreach($items as $item)
            {
                $isExists = $distanceStores->where('id', $item->id)->first();

                if(isset($isExists))
                {
                    $item->distance = $isExists->distance;
                }

                $stores[] = $item;
            }

            $items = collect($stores);
            $items = $items->sortBy('distance');
        }

        if(isset($items) && count($items))
        {
            $itemsOutput['stores'] = array_values($this->storeTransformer->transformCollection($items));
